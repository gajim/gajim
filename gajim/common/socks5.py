...
class SocksQueue:
    ...
    def start_listener(self, port, sha_str, sha_handler, file_props, typ="sender"):
        """
    ...
    def connect_to_hosts(
        self, account, transport_sid, on_success=None, on_failure=None, receiving=True
    ):
        ...
        for streamhost in streamhosts_to_test:
            if receiving:
                if "candidate_id" in streamhost:
                    log.debug(
                        "Trying to connect as receiver to cid %s",
                        streamhost["candidate_id"],
                    )
                else:
                    log.debug(
                        "Trying to connect as receiver to jid %s", streamhost["jid"]
                    )
                file_props.type_ = "r"
                socks5obj = Socks5ReceiverClient(
                    self.idlequeue, streamhost, transport_sid, file_props
                )
                self.add_sockobj(account, socks5obj)
            else:
                if "candidate_id" in streamhost:
                    log.debug(
                        "Trying to connect as sender to cid %s",
                        streamhost["candidate_id"],
                    )
                else:
                    log.debug(
                        "Trying to connect as sender to jid %s", streamhost["jid"]
                    )
        ...
        if unused_hosts:
            for host in file_props.streamhosts:
                if host["state"] == -2:
                    ...
                    client = Socks5ReceiverClient(
                        self.idlequeue, host, client.sid, file_props
                    )
    ...
    def _connection_refused(self, streamhost, file_props, idx):
        ...
        """
        if "candidate_id" in streamhost:
            log.debug("Connection refused to cid %s", streamhost["candidate_id"])
        else:
    ...
    def send_file(self, file_props, account, mode):
        for key in list(self.senders.keys()):
            if (
                file_props.name in key
                and file_props.transport_sid in key
    ...
    def on_connection_accepted(self, sock, listener):
        sock_hash = hash(sock)
        if listener.file_props.type_ == "s" and not self.isHashInSockObjs(
            self.senders, sock_hash
        ):
            sockobj = Socks5SenderServer(
                self.idlequeue,
                sock_hash,
                self,
                sock[0],
            ...
            self.connected += 1
        if listener.file_props.type_ == "r" and not self.isHashInSockObjs(
            self.readers, sock_hash
        ):
            sh = {}
            sh["host"] = sock[1][0]
            sh["port"] = sock[1][1]
    ...
    def remove_receiver(self, idx, do_disconnect=True, remove_all=False):
        ...
        if idx != -1:
            for key in list(self.readers.keys()):
                if idx in key:
                    self.remove_receiver_by_key(key, do_disconnect=do_disconnect)
                    if not remove_all:
                        break
...
class Socks5:
        ...
        try:
            ...
            if errnum == 111 or self.connect_timeout > 1000:
                self.queue._connection_refused(
                    self.streamhost, self.file_props, self.queue_idx
                )
                self.connected = False
                return None
            # win32 needs this
    ...
    def _get_request_buff(self, msg, command=0x01):
        ...
        buff = struct.pack(
            "!BBBBB%dsBB" % len(msg), 0x05, command, 0x00, 0x03, len(msg), msg, 0, 0
        )
        return buff
    ...
    def _get_sha1_auth(self):
        ...
        return hashlib.sha1(
            ("%s%s%s" % (self.sid, self.initiator, self.target)).encode("utf-8")
        ).hexdigest()
    ...
    def main(self, timeout=0):
        ...
        if self.state == 2:  # read auth response
            ...
            if self.queue.on_success:
                result = self.queue.send_success_reply(self.file_props, self.streamhost)
...
class Socks5SenderClient(Socks5Client, Socks5Sender):
    def __init__(
        ...
        Socks5Client.__init__(
            self, idlequeue, host, port, initiator, target, file_props.transport_sid
        )
        Socks5Sender.__init__(
            self, idlequeue, sock_hash, parent, _sock, host, port, connected, file_props
        )
...
class Socks5SenderServer(Socks5Server, Socks5Sender):
    def __init__(
        ...
        Socks5Server.__init__(
            self, idlequeue, host, port, None, None, file_props.transport_sid
        )
        Socks5Sender.__init__(
            self, idlequeue, sock_hash, parent, _sock, host, port, connected, file_props
        )
...
class Socks5ReceiverClient(Socks5Client, Socks5Receiver):
    def __init__(self, idlequeue, streamhost, transport_sid, file_props=None):
        Socks5Client.__init__(
            ...
            streamhost["host"],
            int(streamhost["port"]),
            streamhost["initiator"],
            streamhost["target"],
            transport_sid,
        )
        Socks5Receiver.__init__(self, idlequeue, streamhost, transport_sid, file_props)
