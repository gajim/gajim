...
class Gateway(BaseModule):
    ...
    def unsubscribe(self, agent: str) -> None:
        ...
        self._con.connection.SendAndCallForResponse(iq, self._on_unsubscribe_result)
        self._con.get_module("Roster").delete_item(agent)

    def _on_unsubscribe_result(
        self, _nbxmpp_client: types.NBXMPPClient, stanza: nbxmpp.Protocol
    ) -> None:
        ...
        app.ged.raise_event(
            AgentRemoved(conn=self._con, agent=agent, jid_list=jid_list)
        )

    def request_gateway_prompt(self, jid: str, prompt: str | None = None) -> None:
        typ_ = "get"
        if prompt:
            typ_ = "set"
    ...
    def _on_prompt_result(self, _nbxmpp_client: types.NBXMPPClient, stanza: Iq) -> None:
