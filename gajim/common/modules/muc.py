...
REJOIN_STATUS_CODES = {StatusCode.REMOVED_SERVICE_SHUTDOWN, StatusCode.REMOVED_ERROR}
...
class MUC(BaseModule):
    ...
    def __init__(self, con: types.Client) -> None:
        ...
        self._joined_users: defaultdict[JID, dict[str, MUCPresenceData]] = defaultdict(
            dict
        )
        ...
        self._sent_voice_requests: dict[JID, list[str]] = defaultdict(list)

    def _on_resume_failed(self, _client: types.Client, _signal_name: str) -> None:
    ...
    def _set_muc_state(self, room_jid: JID, state: MUCJoinedState) -> None:
        try:
            ...
            raise ValueError("set_muc_state() called on unknown muc: %s" % room_jid)
    ...
    def join(
        ...
        if muc_data.state not in (
            ...
            self._log.warning("Can’t join MUC %s, state: %s", jid, muc_data.state)
            return

        disco_info = app.storage.cache.get_last_disco_info(muc_data.jid, max_age=60)
    ...
    def _push_muc_added_event(self, jid: JID, select_chat: bool = False) -> None:

        app.ged.raise_event(
            MucAdded(account=self._account, jid=jid, select_chat=select_chat)
        )
    ...
    def leave(
        ...
        self._con.get_module("Presence").send_presence(
            muc_data.occupant_jid, typ="unavailable", status=reason, caps=False
        )
    ...
    def _unregister_callback(self, task: Task) -> None:
        try:
            ...
            self._log.error(
                "Error while unregistering from MUC: %s", to_user_string(error)
            )
    ...
    def abort_join(self, room_jid: JID) -> None:
        ...
        muc = self._mucs[room_jid]
        self._con.get_module("Presence").send_presence(
            muc.occupant_jid, typ="unavailable", caps=False
        )
    ...
    @staticmethod
    def _apply_config(
        form: SimpleDataForm, config: dict[str, Any] | None = None
    ) -> None:
    ...
    def _on_disco_result_after_config(self, task: Task) -> None:
        ...
        event = events.MUCRoomConfigFinished(timestamp=utc_now())
    ...
    def update_presence(self) -> None:
        ...
        for muc_data in mucs:
            self._con.get_module("Presence").send_presence(
                muc_data.occupant_jid, show=status, status=message, idle_time=idle
            )

    def change_nick(self, room_jid: JID, new_nick: str) -> None:
        status, message, _idle = self._con.get_presence_state()
        self._con.get_module("Presence").send_presence(
            f"{room_jid}/{new_nick}", show=status, status=message
        )

    def _on_error_presence(
        self,
        ...
        if muc_data.state == MUCJoinedState.JOINING:
            if properties.error.condition == "conflict":
                ...
                muc_data.nick += "_"
                self._log.info(
                    "Nickname conflict: %s change to %s", muc_data.jid, muc_data.nick
                )
                self._join(muc_data)
            ...
            event = events.MUCRoomPresenceError(timestamp=utc_now(), error=error)
            assert isinstance(room, GroupchatContact)
            app.storage.events.store(room, event)
            room.notify("room-presence-error", event)

    def _on_muc_user_presence(
        self, _con: types.NBXMPPClient, stanza: Presence, properties: PresenceProperties
    ) -> None:
    ...
    def _on_muc_normal_user_message(
        self, _con: types.NBXMPPClient, stanza: Presence, properties: MessageProperties
    ) -> None:
    ...
    def _room_join_complete(self, muc_data: MUCData) -> None:
        ...
        self._con.get_module("Bookmarks").add_or_modify(
            muc_data.jid, autojoin=True, password=muc_data.password, nick=muc_data.nick
        )
    ...
    def get_voice_requests(self, contact: GroupchatContact) -> list[VoiceRequest]:

        return self._voice_requests.get(contact, [])

    def approve_voice_request(
        self, contact: GroupchatContact, voice_request: VoiceRequest
    ) -> None:
    ...
    def _message_error_received(
        ...
        event = events.MUCRoomVoiceRequestError(
            timestamp=utc_now(), message_id=message_id, error=error
        )
    ...
    def _on_captcha_challenge(
        ...
        if muc_data.state != MUCJoinedState.JOINING:
            self._log.warning(
                "Received captcha request but state != %s", MUCJoinedState.JOINING
            )
            return
    ...
    def send_captcha(self, room_jid: JID, form_node: SimpleDataForm) -> None:
    ...
    def _on_config_change(
        ...
        self._log.info(
            "Received config change: %s %s",
            properties.muc_jid,
            properties.muc_status_codes,
        )

        assert properties.muc_status_codes is not None
        event = events.MUCRoomConfigChanged(
            timestamp=utc_now(), status_codes=properties.muc_status_codes
        )
    ...
    def _on_invite_or_decline(
        ...
        if properties.muc_decline is not None:
            ...
            self._log.info(
                "Invite declined from: %s, reason: %s", data.from_, data.reason
            )

            app.ged.raise_event(MucDecline(account=self._account, **data._asdict()))
            raise nbxmpp.NodeProcessed
    ...
    def _on_disco_result_after_invite(self, task: Task) -> None:
        ...
        app.ged.raise_event(
            MucInvitation(
                account=self._account, info=result.info, **invite_data._asdict()
            )
        )

    def invite(
        self, room: JID, jid: JID, reason: str | None = None, continue_: bool = False
    ) -> str:
        ...
        return self._nbxmpp("MUC").invite(room, jid, password, reason, continue_, type_)

    def moderate_messages(
        self, namespace: str, jid: JID, moderate_ids: Sequence[str], reason: str | None
    ) -> None:
