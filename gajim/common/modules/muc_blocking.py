...
def create_extensions_node(
    ...
    if extensions is None:
        extensions = nbxmpp.Node("extensions")
    else:
        extensions = nbxmpp.Node(node=str(extensions))
...
def parse_blocked_occupants(extension: nbxmpp.Node | None) -> set[str]:
    if extension is None:
        return set()

    blocking = extension.getTag("blocking", namespace=GAJIM_BLOCKING_NS)
    if blocking is None:
        raise ValueError("blocking node missing")
