...
class Presence(BaseModule):
    ...
    def _subscribe_received(
        ...
        self._log.info(
            "Received Subscribe: %s, transport: %s, auto_auth: %s, user_nick: %s",
            properties.jid,
            is_transport,
            auto_auth,
            properties.nickname,
        ...
        status = properties.status or _("I would like to add you to my roster.")
    ...
    def _subscribed_received(
        ...
        app.ged.raise_event(
            SubscribedPresenceReceived(account=self._account, jid=properties.jid)
        )
        raise nbxmpp.NodeProcessed

    def _unsubscribe_received(
        self,
        _con: types.NBXMPPClient,
        _stanza: nbxmpp.protocol.Presence,
        properties: PresenceProperties,
    ...
    def get_presence(
        ...
        if idle_time and app.is_installed("IDLE") and app.settings.get("autoaway"):
            idle_sec = idle.Monitor.get_idle_sec()
            time_ = time.strftime(
                "%Y-%m-%dT%H:%M:%SZ", time.gmtime(time.time() - idle_sec)
            )
