...
class BaseModule(EventHelper):
    ...
    def __init__(
        self, con: types.Client, *args: Any, plugin: bool = False, **kwargs: Any
    ) -> None:
    ...
    def _register_pubsub_handler(self, callback: types.AnyCallableT) -> None:
        handler = StanzaHandler(
            name="message", callback=callback, ns=Namespace.PUBSUB_EVENT, priority=49
        )
    ...
    def _get_contact(self, jid: JID, groupchat: bool = False) -> types.ContactT:
        return self._con.get_module("Contacts").get_contact(jid, groupchat=groupchat)
    ...
    def _nbxmpp(
        self, name: NBXMPPModuleNameT | None = None
