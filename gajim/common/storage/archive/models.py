...
class Marker(MappedAsDataclass, Base, UtilMixin, kw_only=True):
    __tablename__ = 'marker'
    __index_cols__ = ['fk_remote_pk', 'fk_occupant_pk', 'fk_account_pk', 'type']
    __upsert_cols__ = ['id', 'timestamp']
    __table_args__ = (Index('idx_marker', *__index_cols__),)
    __no_table_cols__ = ['account_', 'remote_jid_', 'occupant', 'occupant_']
    ...
    def needs_update(self, existing: Marker) -> bool:
    ...
    def get_select_stmt(self) -> Select[Any]:
        return select(Marker).where(
            Marker.fk_remote_pk == self.fk_remote_pk,
            Marker.fk_occupant_pk.is_(self.fk_occupant_pk),
            Marker.fk_account_pk == self.fk_account_pk,
    ...
    markers: Mapped[list[Marker]] = relationship(
        ...
        primaryjoin=sa.and_(
            expr.case(
                (
                    sa.orm.foreign(type) == 2,  # Groupchat
                    sa.orm.foreign(stanza_id) == sa.orm.remote(Marker.id),
                ),
                else_=sa.orm.foreign(id) == sa.orm.remote(Marker.id),
            ),
            fk_remote_pk == Marker.fk_remote_pk,
            fk_account_pk == Marker.fk_account_pk,
        ),
        order_by=Marker.type,
        viewonly=True,
        uselist=True,
    )
