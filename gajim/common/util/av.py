            ...
            if hasattr(element.props, "device"):
                ...
                if devices:
                    ...
                    for device in devices:
                        ...
                        self.devices[text % device_name] = (
                            pipe % f"{name} device={device}"
                        )
                element.set_state(Gst.State.NULL)
            else:
...
class AudioInputManager(DeviceManager):
    def detect(self) -> None:
        ...
        # Test src
        self.detect_element(
            "audiotestsrc", _("Audio test"), "%s is-live=true name=gajim_vol"
        )
        # Auto src
        self.detect_element(
            "autoaudiosrc", _("Autodetect"), "%s ! volume name=gajim_vol"
        )
        # Alsa src
        self.detect_element("alsasrc", _("ALSA: %s"), "%s ! volume name=gajim_vol")
        # Pulseaudio src
        self.detect_element("pulsesrc", _("Pulse: %s"), "%s ! volume name=gajim_vol")
