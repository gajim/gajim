...
iunreserved = rf"{unreserved}{ucschar}"
urlchar = r"A-Za-z0-9\-._~:/?#\[\]@!$&'()*+,;="
pct_loose = rf"%[{urlchar}]"
pct_encoded = rf"%{HEXDIG}{HEXDIG}"
ipchar_loose = rf"(?:[{iunreserved}{sub_delims}:@]|{pct_loose})"
ipchar = rf"(?:[{iunreserved}{sub_delims}:@]|{pct_encoded})"
iuserinfo = rf"(?:[{iunreserved}{sub_delims}:]|{pct_encoded})*"
ireg_name = rf"(?:[{iunreserved}{sub_delims}]|{pct_encoded})*"
dec_octet = (
    r"(?:25[0-5]"
    r"|2[0-4][0-9]"
    r"|1[0-9][0-9]"
...
IPv4address = rf"{dec_octet}(?:\.{dec_octet}){{3}}"
h16 = rf"{HEXDIG}{{1,4}}"
...
IPv6address = (
    ...
    rf"|::(?:{h16}:){{5}}{ls32}"
    rf"|(?:{h16})?::(?:{h16}:){{4}}{ls32}"
    rf"|(?:(?:{h16}:){{,1}}{h16})?::(?:{h16}:){{3}}{ls32}"
    rf"|(?:(?:{h16}:){{,2}}{h16})?::(?:{h16}:){{2}}{ls32}"
    rf"|(?:(?:{h16}:){{,3}}{h16})?::{h16}:{ls32}"
    rf"|(?:(?:{h16}:){{,4}}{h16})?::{ls32}"
    rf"|(?:(?:{h16}:){{,5}}{h16})?::{h16}"
    rf"|(?:(?:{h16}:){{,6}}{h16})?::)"
)
ZoneID = rf"(?:{unreserved}|{pct_encoded})+"
IPv6addrz = rf"{IPv6address}%25{ZoneID}"
IPvFuture = rf"[Vv]{HEXDIG}+\.[{unreserved}{sub_delims}:]+"
IP_literal = rf"\[(?:{IPv6address}|{IPv6addrz}|{IPvFuture})\]"
...
ihost = rf"(?P<host>{IP_literal}|{ireg_name})"
port = r"[0-9]*"
iauthority = rf"(?:{iuserinfo}@)?{ihost}(?::{port})?"
isegment = rf"{ipchar}*"
isegment_nz = rf"{ipchar}+"
ipath_abempty = rf"(?:/{isegment})*"
ipath_absolute = rf"/(?:{isegment_nz}(?:/{isegment})*)?"
ipath_rootless = rf"{isegment_nz}(?:/{isegment})*"
ipath_empty = r""
ihier_part = (
    rf"(?://{iauthority}{ipath_abempty}"
    rf"|{ipath_absolute}"
    rf"|{ipath_rootless}"
    rf"|{ipath_empty})"
)
iquery = rf"(?:{ipchar_loose}|[/?{iprivate}])*"
ifragment = rf"(?:{ipchar}|[/?])*"
scheme = rf"(?P<scheme>{ALPHA}[a-zA-Z0-9+.-]*)"
IRI = rf"{scheme}:{ihier_part}(?:\?{iquery})?(?:#{ifragment})?"
HTTPS_URL = rf"https://{ihier_part}(?:\?{iquery})?(?:#{ifragment})?"
...
class PRECISIdentifierClass:
    _p = PRECIS
    Valid = rf"{_p.LetterDigits}\x21-\x7e"
    Disallowed = rf"{_p.Spaces}{_p.Symbols}{_p.HasCompat}"  # there's a ton more
...
class XMPP:
    ...
    _excl = "\"&'/:<>@"  # <#section-3.3.1>
    localpart = rf"(?:(?![{_excl}{_id.Disallowed}])[{_id.Valid}])+"
    # ^ doesn't take into account "contextual rules" <rfc7564#section-4.2.2>
    ifqdn = rf"[{iunreserved}{sub_delims}]+"
    # ^ probably correct
    domainpart = rf"(?:{IP_literal}|{IPv4address}|{ifqdn})"
    jid = rf"(?P<local>{localpart})@(?P<domain>{domainpart})"
