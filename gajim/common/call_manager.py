...
class CallManager(EventHelper):
    ...
    def _on_jingle_request(self, event: events.JingleRequestReceived) -> None:
        ...
        if self._resource_jid is not None:
            if str(self._resource_jid) != event.fjid:
                log.info(
                    "Received jingle request while in active call: %s, %s",
                    event.account,
                    event.jid,
                )
                return
        ...
        if "audio" in content_types:
            self._set_jingle_state("audio", JingleState.CONNECTION_RECEIVED, event.sid)
        if "video" in content_types:
            self._set_jingle_state("video", JingleState.CONNECTION_RECEIVED, event.sid)

        if (
            self._jingle_audio_state in (JingleState.CONNECTED, JingleState.CONNECTING)
    ...
    def _on_jingle_connected(self, event: events.JingleConnectedReceived) -> None:
        client = app.get_client(event.account)
        session = client.get_module("Jingle").get_jingle_session(event.fjid, event.sid)
        if session is None:
            return

        if event.media == "audio":
            self._set_jingle_state("audio", JingleState.CONNECTED, event.sid)
        if event.media == "video":
            self._set_jingle_state("video", JingleState.CONNECTED, event.sid)
    ...
    def _on_jingle_disconnected(self, event: events.JingleDisconnectedReceived) -> None:
        ...
        if event.media == "audio":
            self._set_jingle_state(
                "audio", JingleState.NULL, sid=event.sid, reason=event.reason
            )
        if event.media == "video":
            self._set_jingle_state(
                "video", JingleState.NULL, sid=event.sid, reason=event.reason
            )
        sound.stop()

    def _on_jingle_error(self, event: events.JingleErrorReceived) -> None:
        if event.sid == self._jingle_audio_sid:
            self._set_jingle_state("audio", JingleState.ERROR, reason=event.reason)
        sound.stop()

    def _set_jingle_state(
        self,
        jingle_type: str,
        state: JingleState,
        sid: str | None = None,
        ...
        if jingle_type == "audio":
            if state == self._jingle_audio_state:
                return
            if state == JingleState.NULL and self._jingle_audio_sid not in (None, sid):
                return
        else:
            if state == self._jingle_video_state:
                return
            if state == JingleState.NULL and self._jingle_video_sid not in (None, sid):
                return
        ...
        if (
            ...
            and self._jingle_video_state == JingleState.NULL
        ):
            assert self._account is not None
            assert self._resource_jid is not None
            app.ged.raise_event(
                events.CallStopped(
                    self._account, JID.from_string(self._resource_jid.bare)
                )
            )
    ...
    def _get_audio_content(self, account: str, jid: JID) -> JingleAudio | None:
        client = app.get_client(account)
        session = client.get_module("Jingle").get_jingle_session(
    ...
    def start_call(self, account: str, jid: JID, call_type: CallType) -> None:
        client = app.get_client(account)
        ...
        if self._jingle_audio_sid is None:
            app.ged.raise_event(events.CallStarted(self._account, self._resource_jid))

        if call_type == CallType.VIDEO:
            sid = client.get_module("Jingle").start_audio_video(str(self._resource_jid))
        ...
        if self._jingle_audio_state != JingleState.NULL:
            ...
            sid = client.get_module("Jingle").start_audio(str(self._resource_jid))
            self._set_jingle_state("audio", JingleState.CONNECTING, sid)
            self._store_outgoing_call(account, jid, sid)

    def stop_call(self, account: str, contact: types.ResourceContact) -> None:
        self._close_jingle_content(account, contact.jid, "audio", shutdown=True)
        self._close_jingle_content(account, contact.jid, "video", shutdown=True)
    ...
    def accept_call(self, session: JingleSession) -> None:
        ...
        if audio and not audio.negotiated:
            self._set_jingle_state("audio", JingleState.CONNECTING, session.sid)
        if video and not video.negotiated:
            self._set_jingle_state("video", JingleState.CONNECTING, session.sid)
    ...
    @staticmethod
    def _get_call_resources(contact: types.BareContact) -> list[types.ResourceContact]:
