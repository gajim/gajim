...
def get_usage_from_command(cmd: argparse.ArgumentParser, command_name: str) -> str:
...
class ChatCommands(Observable):
    ...
    def make_parser(
        self, command_name: str, callback: Callable[..., Any], **kwargs: Any
    ) -> ArgumentParser:
        """Add and return a subparser and initialize
        it with the command name.
        """

        parser = self._sub_parser.add_parser(command_name, add_help=False, **kwargs)
        parser.set_defaults(command_name=command_name, exec=callback)
        return parser

    def add_command(
        self, command_name: str, used_in: list[str], cmd: argparse.ArgumentParser
    ) -> None:
    ...
    def parse(self, type_: str, arg_string: str) -> bool:
        ...
        if command is None or type_ not in command[0]:
            ...
            self.notify("command-not-found", f"{unknown_command}\n{hint}")
            raise CommandFailed
    ...
    def _create_commands(self) -> None:
        ...
        parser.add_argument("status", choices=["online", "away", "xa", "dnd"])
        ...
        parser.add_argument("who")
        parser.add_argument("affiliation", choices=["owner", "admin", "member", "none"])
        ...
        parser.add_argument("who")
        parser.add_argument("role", choices=["moderator", "participant", "visitor"])
    ...
    def _check_if_joined(self) -> GroupchatContact:
        ...
        if not contact.is_joined:
            raise CommandError(_("You are currently not joined this group chat"))
        return contact
    ...
    def _change_affiliation(
        self, nick_or_address: str, affiliation: str, reason: str | None
    ) -> None:
        ...
        if nick_or_address in nick_list:
            ...
            if not is_affiliation_change_allowed(self_contact, participant, "outcast"):
        ...
        client.get_module("MUC").set_affiliation(
            contact.jid, {jid: {"affiliation": affiliation, "reason": reason}}
        )
    ...
    def _change_role(self, nick: str, role: str, reason: str | None) -> None:
        ...
        client = app.get_client(contact.account)
        client.get_module("MUC").set_role(contact.jid, nick, role, reason)
