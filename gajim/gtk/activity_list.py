...
# TODOs
# overview: https://dev.gajim.org/gajim/gajim/-/issues/11677
# check for self._client availability (online if necessary), see https://dev.gajim.org/gajim/gajim/-/issues/11768

# messages where we have been mentioned in group chats (see https://dev.gajim.org/gajim/gajim/-/issues/11209)
...
# (see https://dev.gajim.org/gajim/gajim/-/issues/11592 and
# https://dev.gajim.org/gajim/gajim/-/issues/11591 )
# Tabs (unread counter for each one):
#   Activity Stream, App Notifications, Reminders
...
class ActivityList(Gtk.ListBox, EventHelper):
    ...
    def remove_activities_for_account(self, account: str) -> None:
        for row in cast(list[ActivityListRow], self.get_children()):
    ...
    def _on_destroy(self, _activity_list: ActivityList) -> None:
    ...
    def _update_unread(self) -> None:
        rows = cast(list[ActivityListRow], self.get_children())
    ...
    def _on_row_destroy(self, _row: ActivityListRow) -> None:
        self._update_unread()
...
class ActivityListRow(Gtk.ListBoxRow):
    def __init__(self, account: str | None = None, jid: JID | None = None) -> None:
        ...
        self.add(self._ui.activity_list_row)

        self.get_style_context().add_class('activitylist-row')
    ...
    def set_avatar_from_gajim_icon(self) -> None:
        scale = self.get_scale_factor()
        surface = app.app.avatar_storage.get_gajim_circle_icon(AvatarSize.ROSTER, scale)
        self._ui.avatar_image.set_from_surface(surface)
    ...
    def mark_as_read(self) -> None:
        self._is_read = True
        self._ui.activity_title_label.get_style_context().remove_class('bold')

    def mark_as_unread(self) -> None:
        self._is_read = False
        self._ui.activity_title_label.get_style_context().add_class('bold')
...
class GajimUpdatePermissionRow(ActivityListRow):
    def __init__(self) -> None:
        ...
        self._ui.activity_type_indicator.set_from_icon_name(
...
class SubscribeRow(ActivityListRow):
    def __init__(self, event: events.SubscribePresenceReceived) -> None:
        ...
        self._ui.activity_type_indicator.set_from_icon_name(
            'feather-user-symbolic', Gtk.IconSize.MENU
        )
        self._ui.activity_title_label.set_text(self.get_title_text())
        self._ui.activity_subject_label.set_text(self.get_subject_text())
...
class MucInvitationReceivedRow(ActivityListRow):
    def __init__(self, event: events.MucInvitation) -> None:
        ...
        self._ui.activity_type_indicator.set_from_icon_name(
            'feather-users-symbolic', Gtk.IconSize.MENU
        )
        self._ui.activity_title_label.set_text(self.get_title_text())
        self._ui.activity_subject_label.set_text(self.get_subject_text())
