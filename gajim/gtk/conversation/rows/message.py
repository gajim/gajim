...
class MessageRow(BaseRow):
    ...
    def update_with_content(self, message: Message) -> None:
        ...
        # Markers are always for the original message, never the correction
        marker = self._original_message.get_latest_marker()
        if (marker is not None and
                not self._contact.is_groupchat and
                self.direction == ChatDirection.OUTGOING and
