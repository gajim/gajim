...
class GroupchatAffiliation(Gtk.Box, SignalManager):
    def __init__(self, client: Client, contact: GroupchatContact) -> None:
        ...
        self._connect(
            self._ui.affiliation_selection, "changed", self._on_selection_changed
        )
        ...
        self._connect(
            self._ui.affiliation_renderer, "changed", self._on_affiliation_changed
        )
    ...
    def _on_add(self, _button: Gtk.Button) -> None:
        ...
        iter_ = self._store.append(
            [None, None, "member", _("Member"), affiliation_edit, jid_edit]
        )
    ...
    def _on_remove(self, _button: Gtk.Button) -> None:
        ...
        owner_count = len(
            [row for row in self._store if row[Column.AFFILIATION] == "owner"]
        )
            ...
            if self._store[path][Column.AFFILIATION] == "owner":
                if owner_count < 2:
                    ...
                    InformationAlertDialog(
                        _("Error"), _("A Group Chat needs at least one Owner")
                    )
                    return
                owner_count -= 1
    ...
    @staticmethod
    def _get_selected_affiliations(tree_selection: Gtk.TreeSelection) -> set[str]:
    ...
    def _on_affiliations_received(self, task: Task) -> None:
        ...
        try:
            ...
            log.info(
                "Error while requesting %s affiliations: %s",
                affiliation,
                error.condition,
            )
            return
