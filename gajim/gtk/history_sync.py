...
class HistorySyncAssistant(Assistant, EventHelper):
    ...
    def _received_count(self, task: Task) -> None:
        ...
        self._query_id = mam_module.request_archive_interval(self._start, self._end)
    ...
    def set_fraction(self) -> None:
        ...
        if self._count:
            ...
            self._progress_bar.set_text(
                ...
                % {"received": self._received, "max": self._count}
            )
        else:
            self._progress_bar.pulse()
            self._progress_bar.set_text(_("Downloaded %s messages") % self._received)
